...
const FALLBACK_PATTERNS = [
  ...
  {
    // Prefix: จำไว้ว่า.../จดจำ.../remember.../จดไว้.../อย่าลืม.../บันทึกไว้.../บันทึก.../note ไว้...
    pattern: /(?:จำไว้|จดจำ|จำด้วย|จดไว้|อย่าลืม|บันทึกไว้|บันทึก|remember|note\s*ไว้)\s*(?:ว่า\s*|นะ\s*|ด้วย\s*|ที่\s*)?(.+)/i,
    tool: 'save_memory',
    extract: (m) => ({ content: m[1].trim() }),
  },
  {
    // Postfix: "ร้านนี้ดี จำด้วยนะ" / "...อย่าลืมนะ" / "...จำไว้ด้วย"
    pattern: /^(.+?)\s+(?:จำด้วย|จำไว้ด้วย|อย่าลืม|จดไว้ด้วย|บันทึกด้วย)(?:นะ|ด้วย|นะคะ|ครับ|ค่ะ)?$/i,
    tool: 'save_memory',
    extract: (m) => ({ content: m[1].trim() }),
  },
];
